(() => {
  ...
  function navigation(){
    ...
    const proposal=$('proposal');if(proposal){
      const hero=proposal.querySelector('.v28-hero');if(hero&&!hero.querySelector('.v69-phase2-back')){
        const b=document.createElement('button');b.type='button';b.className='v69-phase2-back';b.textContent='← Volver al panel del PCI';b.onclick=()=>go('panel');hero.prepend(b)
      }
  ...
  function refresh(){clearTimeout(timer);timer=setTimeout(()=>{navigation();ensureImport();accordions();patchPrint()},90)}
  ...
  .v69-back-btn,.v69-phase2-back{border:1px solid var(--line);background:#fff;color:var(--ink);padding:8px 12px;border-radius:999px;font-weight:900;font-size:.68rem;box-shadow:0 5px 14px rgba(18,57,92,.08)}
  .v69-phase2-back{position:relative;z-index:3;margin-bottom:10px}.v69-existing-back{display:inline-flex!important;align-items:center;min-height:34px;padding:7px 11px!important;border:1px solid var(--line)!important;border-radius:999px;background:#fff!important}
