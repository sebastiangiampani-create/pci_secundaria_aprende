(() => {
  ...
  function offer(tid,sem){
    ...
    const fallback=Math.min(50,Math.max(0,minPct));
    const selected=Number.isFinite(stored)?clamp(stored,0,50):fallback;
    const effective=conflict?50:Math.max(minPct,selected);
    const targetOutside=conflict?minimumOutside:Math.max(minimumOutside,Math.floor(front*effective/100));
    const configuredTotal=front+targetOutside;
    const residual=Math.max(0,targetOutside-team-explicit);
    return{front,team,explicit,minimumOutside,minimumTotal,maxOutside,maxTotal,minPct,selected:effective,targetOutside,configuredTotal,residual,conflict};
  }
  ...
  style.textContent=`
    ...
    .v56-choice{display:grid;grid-template-columns:1fr 112px;gap:8px;align-items:end;margin-top:9px}.v56-choice label{display:grid;gap:3px;font-size:.57rem;font-weight:850;color:var(--muted)}.v56-choice select{width:100%;padding:8px;border:1px solid var(--line);border-radius:8px;background:#fff;color:var(--ink);font-weight:850}.v56-result{padding:8px 9px;border-radius:9px;background:var(--band);font-size:.6rem;line-height:1.35}.v56-result strong{display:block;font-size:.72rem;color:var(--ink)}
  ...
  function teacherCard(t){
    const semesters=[1,2].map(sem=>{const o=offer(t.id,sem);return`<div class="v56-sem"><div class="v56-sem-head"><span>${sem===1?'1.er':'2.º'} cuatrimestre</span><span>${o.front} HC frente a curso</span></div><div class="v56-metrics"><span>Planificación/equipo obligatoria</span><strong>${o.team} HC</strong>${o.explicit?`<span>Otras tareas institucionales ya definidas</span><strong>${o.explicit} HC</strong>`:''}<span class="v56-min">Oferta mínima total</span><strong>${o.minimumTotal} HC</strong><span class="v56-max">Máximo posible con 50 %</span><strong>${o.maxTotal} HC</strong></div>${o.front?`<div class="v56-choice"><label>Porcentaje total fuera de curso sobre las HC frente a curso<select data-v56-pct="${esc(t.id)}" data-sem="${sem}" ${o.conflict?'disabled':''}>${percentageOptions(o.minPct,o.selected)}</select></label><div class="v56-result"><strong>${o.configuredTotal} HC totales</strong>${o.targetOutside} HC fuera de curso${o.residual?` · ${o.residual} HC institucionales flexibles`:''}</div></div>`:'<div class="v56-result" style="margin-top:9px"><strong>Sin horas frente a curso asignadas</strong>La oferta se calcula cuando exista carga docente.</div>'}${o.conflict?`<div class="v56-conflict">Las ${o.minimumOutside} HC obligatorias fuera de curso superan el 50 % de una carga frente a curso de ${o.front} HC. El sistema conserva el mínimo obligatorio y marca esta situación para revisión.</div>`:''}</div>`}).join('');
  ...
  function syntheticOutsideRows(sem){
    ...
    for(const t of teachers()){
      ...
      if(o.residual>0)rows.push({id:`offer-extra-${t.id}-S${sem}`,teacherId:t.id,label:'Trabajo institucional adicional',hours:o.residual,semester:sem,__v56Offer:true});
    }
    return rows;
  }
  ...
  function render(){
    ...
    try{
      ...
      section.innerHTML=`<div class="eyebrow">Carga y ofrecimiento docente</div><h2>Oferta mínima, porcentaje variable y trabajo en equipo</h2><p>Primero se calcula la carga frente a curso. Luego se suman obligatoriamente las horas de planificación de los equipos a los que pertenece cada docente. El trabajo fuera de curso puede ampliarse hasta un máximo equivalente al 50 % de las horas frente a curso; ese porcentaje no se asigna automáticamente.</p><div class="v56-note"><strong>Mínimo automático:</strong> horas frente a curso + planificación/equipo obligatoria. <strong>Máximo:</strong> horas frente a curso + hasta 50 % de trabajo fuera de curso. Si un docente integra más de un equipo, se suman los bloques obligatorios reales de todos ellos.</div>${teachers().length?`<div class="v56-cards">${teachers().map(teacherCard).join('')}</div>`:'<div class="v48-empty">Primero cargá docentes y asignales materias/cursos.</div>'}<h2 style="margin-top:19px">Bloques obligatorios de planificación</h2><p>Los equipos se detectan automáticamente desde las asignaciones de Fase 1. Cada área de Formación General y cada Formación Orientada debe tener un bloque semanal común continuo de 3 o 4 HC dentro de la jornada escolar.</p>${teamCards()}`;
