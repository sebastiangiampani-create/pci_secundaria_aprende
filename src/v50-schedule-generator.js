(() => {
  ...
  const days=()=>availability()?.DAYS||[['mon','Lunes'],['tue','Martes'],['wed','Miércoles'],['thu','Jueves'],['fri','Viernes']];
  const periods=()=>availability()?.periods?.()||8;
  const slotKey=(day,p)=>`${day}:${p}`;
  const maxForTeacher=t=>Number(t?.baseHours||0)*(1+Number(t?.extraPct??50)/100);

  const style=document.createElement('style');style.textContent=`
    .v50-toolbar{display:flex;gap:8px;align-items:end;flex-wrap:wrap;margin-top:12px}.v50-toolbar label{display:grid;gap:4px;font-size:.62rem;font-weight:850}.v50-toolbar select{padding:8px 9px;border:1px solid var(--line);border-radius:9px;background:#fff;color:var(--ink)}
    ...
    .v50-grid-wrap{overflow:auto;margin-top:10px;border:1px solid var(--line);border-radius:13px}.v50-grid{width:100%;min-width:820px;border-collapse:collapse;table-layout:fixed;font-size:.61rem}.v50-grid th{padding:8px;background:var(--band);font-size:.56rem;text-transform:uppercase;letter-spacing:.04em}.v50-grid td{height:56px;padding:5px;border-top:1px solid var(--line);border-right:1px solid var(--line);vertical-align:top}.v50-grid td:last-child{border-right:0}.v50-grid .period{width:68px;text-align:center;font-weight:900;background:#fafcfd}.v50-cell strong{display:block;font-size:.62rem;line-height:1.2}.v50-cell small{display:block;margin-top:3px;color:var(--muted);font-size:.53rem;line-height:1.25}
  ...
  function preflight(){
    const rows=demandRows(),root=inst(),teachers=root.teachers||{},issues=[],warnings=[];
    ...
    const courseMap=new Map(),teacherMap=new Map();
    for(const r of rows){
    ...
    for(const [course,h] of courseMap)if(h>capacity)issues.push(`${course.replace('|',' · ')} requiere ${h} módulos y la grilla dispone de ${capacity}.`);
    for(const [tid,h] of teacherMap){
      ...
      if(h>free)issues.push(`${t.name} necesita ${h} módulos en este cuatrimestre pero declaró solo ${free} disponibles.`);
      const max=maxForTeacher(t);if(max&&h>max)issues.push(`${t.name} tiene ${h} módulos semanales y supera su máximo ofrecible de ${max.toFixed(1).replace('.0','')}.`);
      else if(max&&h>max*.85)warnings.push(`${t.name} utiliza ${h} de ${max.toFixed(1).replace('.0','')} módulos máximos ofrecibles.`);
    }
  ...
  function buildUnits(report){
    ...
    const units=[];
    report.rows.forEach(r=>{for(let i=0;i<Number(r.hours);i++)units.push({...r,unitIndex:i,courseKey:`${r.orientation}|${r.course}`,difficulty:teacherSlack[r.teacherId]??999})});
    return units;
  }
  ...
  function attempt(report){
    ...
    for(const unit of units){
      unit.__placed=placedByInstance.get(unit.instanceId)||[];
      const pick=chooseSlot(unit,courseOcc,teacherOcc,instanceDayCount,courseDayLoad,teacherDayLoad);if(!pick)return{ok:false,entries,unresolved:units.slice(entries.length)};
      ...
      const dk=`${unit.instanceId}|${pick.day}`;instanceDayCount.set(dk,(instanceDayCount.get(dk)||0)+1);
      const ck=`${unit.courseKey}|${pick.day}`;courseDayLoad.set(ck,(courseDayLoad.get(ck)||0)+1);
      const tk=`${unit.teacherId}|${pick.day}`;teacherDayLoad.set(tk,(teacherDayLoad.get(tk)||0)+1);
  ...
  function quality(entries){
    ...
    for(const ps of byTeacherDay.values()){ps.sort((a,b)=>a-b);if(ps.length>1)score+=(ps[ps.length-1]-ps[0]+1-ps.length)*1.2}
    return score;
  }
  ...
  function generate(){
    ...
    for(let i=0;i<180;i++){
      ...
      if(result.ok){const q=quality(result.entries);if(!best||q<best.quality)best={...result,quality:q};if(q<=1)break}
      else if(!bestPartial||result.entries.length>bestPartial.entries.length)bestPartial=result;
    }
    if(!best){lastReport={...report,ok:false,issues:[...report.issues,`No se encontró una combinación completa. La mejor tentativa ubicó ${bestPartial?.entries.length||0} de ${buildUnits(report).length} módulos. Revisá disponibilidades o asignaciones.`]};renderSection();return}
    const schedule={id:`sch-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,semester,createdAt:new Date().toISOString(),status:'draft',periods:periods(),days:days().map(x=>x[0]),entries:best.entries,quality:best.quality};
  ...
  function reportHtml(report){if(!report)return'';const cls=report.ok?'ok':'bad',title=report.ok?'La carga es programable con las restricciones actuales':'Hay condiciones que impiden generar un horario completo';return`<div class="v50-report ${cls}"><h3>${title}</h3>${report.issues?.length?`<ul>${report.issues.map(x=>`<li>${esc2(x)}</li>`).join('')}</ul>`:'<p>No se detectaron conflictos duros antes de generar.</p>'}${report.warnings?.length?`<div class="v50-diagnostic">${report.warnings.map(x=>`⚠ ${esc2(x)}`).join('<br>')}</div>`:''}</div>`}

  function scheduleView(schedule){
    if(!schedule)return'<div class="v48-empty">Todavía no hay un horario generado para este cuatrimestre.</div>';
    const entries=schedule.entries||[],options=viewMode==='course'?[...new Map(entries.map(e=>[e.courseKey,`${e.orientation} · ${e.course}`])).entries()]:[...new Map(entries.map(e=>[e.teacherId,e.teacherName])).entries()];
