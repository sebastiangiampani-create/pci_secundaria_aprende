(() => {
  ...
  let commission='',date=new Date().toISOString().slice(0,10);
  ...
  function defs(){return window.PCIStudentsCommissionsV72?.commissionDefs?.()||[]}
  ...
  function del(id){root().records=root().records.filter(r=>r.id!==id);save();render()}
  function ensure(){
  ...
  function patchHome(){
    ...
    card.innerHTML='<div><div class="eyebrow">5 · Asistencia</div><h2>Asistencia y regularidad</h2><p>Registro diario por estudiante, justificación y seguimiento de la condición de regularidad.</p><small>Alimenta automáticamente la condición que luego se muestra en el boletín.</small></div><button type="button" class="btn primary" data-v78-open>Abrir Asistencia</button>';
    card.querySelector('[data-v78-open]')?.addEventListener('click',open);
  }
  function render(){const h=host();if(!h)return;const ds=defs();if(!commission&&ds[0])commission=ds[0].key;const ss=students(commission),rs=root().records.filter(r=>r.commissionKey===commission&&r.date===date);
